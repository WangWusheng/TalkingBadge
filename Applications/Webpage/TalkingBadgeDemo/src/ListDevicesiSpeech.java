 ...
 */
@WebServlet("/ListDevicesiSpeech")
public class ListDevicesiSpeech extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		// out.println("<script>function open_win(){window.open(\"PreListen\");}</script>");
		...
		if (!TBService.validateUser(Username, Password)) {
			...
			out.println("<input type=\"submit\" disabled=\"disabled\" value=\"Send a message to selected users\"/></form>");
			...
			out.println("<input type=\"hidden\" name=\"Password\" value=\""
					+ Password + "\"/>");
			out.println("<input type=\"submit\"  value=\"Send a message for people who are currently at specific locations\"/></form>");

			out.println("<br><h2><font size=\"2\" face=\"verdana\" color=\"#800080\">Select one or more users that you want to send message to:</font></h2><br>");
			...
			out.println("<br><h2><font size=\"2\" face=\"verdana\" color=\"#800080\">Language: <br><br></font></h2><select name=\"Language\"> <option value =\"ukenglish\">English (UK)</option> <option value =\"usenglish\">English (USA)</option> "
			...
			// + "</select><br>");
			// out.println("<input name=\"apikey\" type=\"hidden\"  value=\"developerdemokeydeveloperdemokey\"><input name=\"action\" type=\"hidden\" value=\"convert\">");
			// out.println("<br><button type=\"button\" onclick=\"selectUserForm.action='PreListen';selectUserForm.submit();\">Listen</button>");
			out.println("If you would like to hear your message before you send it then just copy it from the textfield and paste it into this demo: <a href=http://www.ispeech.org/text.to.speech>http://www.ispeech.org/text.to.speech</a><br> Remember to choose the right language. You will only hear the first 90 characters of your message, though.");
