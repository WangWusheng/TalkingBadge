package com.blipsystems.blipnet.samples.statusmonitor;
...
public class StatusMonitorSample implements BlipNetSample, SessionEventListener, IpAddressEventListener, ConnectionEventListener {
    ...
    public void startSample(BlipNodeHandle handle, ConnectData connectData) {
        ...
        try {
            ...
            handle.getConnectionHandler().addEventListener(this);
            connectData.setRssiMeasurementOn(true);
            connectData.setLinkQualityMeasurementOn(true);
            connectData.setAfhChannelMapReportingOn(true);
            connectData.setStatusMeasurementInterval(1000);
    ...
    public void handleConnectionEvent(ConnectionEvent event) {
        if (event.getEventID() == ConnectionEvent.CONNECTION_STATUS) {
            System.out.println("Status info: RSSI = " + event.getRssiResult() + " - BER: " + event.getBitErrorRate() +
                    " - Channel Map: " + event.getAfhChannelMap());
        }
    }
