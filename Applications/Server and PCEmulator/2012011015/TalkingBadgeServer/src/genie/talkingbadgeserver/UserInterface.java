...
public class UserInterface extends JFrame {
	...
	UserInterface() {
		...
		final String fileNameTextInitWord = "Input the file name here, without path or expanded-name.";
	...
	protected static String text2sound(String name, String text) {
		...
		try {
			...
			AudioFileSink sink = new AudioFileSink(new File(outPutFile), ff);
	...
	private class SendFileEventListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (fileNameText.getText().replaceAll(" ", "").isEmpty()) {
				JOptionPane.showConfirmDialog(null, "Invalid file name.",
				...
				final String theFilepath = text2sound(fileNameText.getText(),
				...
				if (!theFile.exists()) {
					...
					try {
						...
						handle.getOppClientHandler().push(selectedUD.getCd(),
								opo);
					} catch (Exception e1) {
						e1.printStackTrace();
					}
